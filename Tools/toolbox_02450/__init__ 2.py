...
def glm_validate(X, y, cvf=10):
    ...
    for train_index, test_index in CV.split(X):
        ...
        validation_error[f] = np.square(
            y_test-m.predict(X_test)).sum()/y_test.shape[0]
        f = f+1
    ...
    for feature in range(0, X.shape[1]):
        if np.where(selected_features == feature)[0].size == 0:
            trial_selected = np.concatenate(
                (selected_features, np.array([feature])), 0).astype(int)
...
def rlr_validate(X, y, lambdas, cvf=10):
    ...
    for train_index, test_index in CV.split(X, y):
        ...
        for l in range(0, len(lambdas)):
            ...
            train_error[f, l] = np.power(
                y_train-X_train @ w[:, f, l].T, 2).mean(axis=0)
            test_error[f, l] = np.power(
                y_test-X_test @ w[:, f, l].T, 2).mean(axis=0)
...
def dbplotf(X, y, fun, grid_range, resolution=100.0):
    ...
    if grid_range == 'auto':
        grid_range = [X.min(axis=0)[0], X.max(axis=0)[0],
    ...
    coords = np.mat([[f1[i], f2[j]] for i in range(len(f1))
    ...
    for c in range(C):
        cmask = (y == c)
        plt.plot(X[cmask, 0], X[cmask, 1], '.',
                 color=C_colors[c], markersize=10)
    plt.title('Model prediction and decision boundary')
    plt.xlabel('Feature 1')
    plt.ylabel('Feature 2')
    plt.contour(F1, F2, values, levels=C_levels,
                colors=['k'], linestyles='dashed')
    plt.contourf(F1, F2, values, levels=np.linspace(
        values.min(), values.max(), levels), cmap=plt.cm.jet, origin='image')
...
def dbplot(classifier, X, y, grid_range, resolution=100):
    ...
    coords = np.array([[f1[i], f2[j]] for i in range(len(f1))
                       for j in range(len(f2))])
...
def dbprobplot(classifier, X, y, grid_range, resolution=100):
    ...
    plt.contour(F1, F2, values, levels=C_levels,
                colors=['k'], linestyles='dashed')
    contourf(F1, F2, values, levels=np.linspace(values.min(),
                                                values.max(), levels), cmap=cm.jet, origin='image')
    plt.colorbar(format='%.1f')
    plt.legend(C_legend)
    # hold(False)
...
def confmatplot(y_true, y_est):
    ...
    plt.title('Confusion matrix (Accuracy: {:}%, Error Rate: {:}%)'.format(
        accuracy, error_rate))
...
def bootstrap(X, y, N, weights='auto'):
    ...
    selected_indices = np.random.choice(
        range(N), size=(N, 1), replace=True, p=weights).flatten()
    if np.ndim(y) == 1:
...
def clusterplot(X, clusterid, centroids='None', y='None', covars='None'):
    ...
    for i, cs in enumerate(np.unique(y)):
        plt.plot(X[(y == cs).ravel(), 0], X[(y == cs).ravel(), 1], 'o',
                 markeredgecolor='k', markerfacecolor=colors[i], markersize=6, zorder=2)
    for i, cr in enumerate(np.unique(cls)):
        plt.plot(X[(cls == cr).ravel(), 0], X[(cls == cr).ravel(), 1], 'o', markersize=12,
                 markeredgecolor=colors[i], markerfacecolor='None', markeredgewidth=3, zorder=1)
    if type(centroids) is not str:
        for cd in range(centroids.shape[0]):
            plt.plot(centroids[cd, 0], centroids[cd, 1], '*', markersize=22,
                     markeredgecolor='k', markerfacecolor=colors[cd], markeredgewidth=2, zorder=3)
    # plot cluster shapes:
    ...
    )+np.unique(cls).tolist()+np.unique(cls).tolist()
...
def train_neural_net(model, loss_fn, X, y,
    ...
    for r in range(n_replicates):
        ...
        print('\t\tFinal loss:')
        print_str = '\t\t' + str(i+1) + '\t' + \
            str(loss_value) + '\t' + str(p_delta_loss)
        print(print_str)
...
def visualize_decision_boundary(predict,
    ...
    if train is not None or test is not None:
        assert not isinstance(
            X, list), 'Cannot provide indices of test and train partition, if X is provided as list of train and test partition.'
        assert not isinstance(
            y, list), 'Cannot provide indices of test and train partition, if y is provided as list of train and test partition.'
        ...
        train_index = np.array([(int(e) in train)
                                for e in np.linspace(0, N-1, N)])
        test_index = np.array([(int(e) in test)
                               for e in np.linspace(0, N-1, N)])
    ...
    for i in range(C):
        ...
        if isinstance(X, list) and isinstance(y, list):
            for j, (X_par, y_par) in enumerate(zip(X, y)):
                ...
                h = plt.plot(X_par[idx, 0], X_par[idx, 1],
                             s[j], color=c[i], alpha=t[j])
...
def draw_neural_net(weights, biases, tf,
    ...
    for n, (layer_size_a, layer_size_b) in enumerate(zip(layer_sizes[:-1], layer_sizes[1:])):
        ...
        for m in range(layer_size_a):
            for o in range(layer_size_b):
                ...
                linewidth = (coefs_[n][m, o] / largest_coef) * \
    ...
    for n, (layer_size_a, layer_size_b) in enumerate(zip(layer_sizes[:-1], layer_sizes[1:])):
        ...
        for o in range(layer_size_b):
            ...
            linewidth = (intercepts_[n][o] / largest_coef) * \
                max_line_width+min_line_width
    ...
    for m in range(layer_sizes[-1]):
        plt.arrow(right+0.015, layer_top_0 - m*v_spacing, 0.16 *
                  h_spacing, 0,  lw=1, head_width=0.01, head_length=0.02)
